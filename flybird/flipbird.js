...
window.onload = function (){
   ...
   //获取上下文(画笔)
	...
	//循环调用一个(函数)
	//setInterval(a,b):a表示循环调用哪个函数,b每隔几ms调用此函数
...
//通过按空格键让小鸟上升的函数
//事件函数里面自带事件对象，使用evt来来代替事件对象
function keyup(evt){

	//兼容浏览器，有些浏览器获取不到事件对象（window.event）
	evt = evt || window.event;
	//键码（ASCII）：代表键盘里面键的值 空格键的键码：32
	//事件对象获取键码
...
function Pipe (x,y,width,height,src){
	...
	this.drawUpPipe = function () {
		//drawImage(图片对象,目标图片的x,目标图片的y,目标图片的width,目标图片的height,x,y,width,height)
   ...
   bird.y += 10;
   		...
   		//与地板碰撞（小鸟的y坐标加上小鸟的高>=地板的y坐标）
   		var floorCondition=bird.y + bird.height >= floor.y + 5;
   		//与上管道的碰撞只需判断左上角与右上角的点即可
   		/* 小鸟左上角x的取值范围：
   		上管道的x <= x <= 上管道的x + 上管道的width
	
		小鸟左上角y的取值范围：
		上管道的y <= y <= 上管道的y + 上管道的height

		小鸟右上角x的取值范围：
   		上管道的x <= x+width <= 上管道的x + 上管道的width
		...
		*/
   		//左上角的取值范围
   		var zuoShangJiaoCondition=(bird.x >= upPipe.x +20 && bird.x <= upPipe.x + upPipe.width) && (bird.y >= 0 && bird.y <= upPipe.y + upPipe.height);  		
   		//右上角取值范围
   		var youShangJiaoCondition=(bird.x + bird.width >= upPipe.x && bird.x + bird.width <= upPipe.x + upPipe.width) && (bird.y >= 0 && bird.y <= upPipe.y + upPipe.height);  		
   		
   		//与下管道的碰撞,只需判断左下角与右下角的点即可
   		/*
		 小鸟左下角x的取值范围：
   		下管道的x <= x <= 下管道的x + 下管道的width
	
		小鸟左下角y的取值范围：
		下管道的y <= y+height <= 下管道的y + 下管道的height

		小鸟右下角x的取值范围：
   		下管道的x <= x+width <= 下管道的x + 下管道的width
		...
		*/
   		...
   		//与天花板碰撞
   		var cellCondition =bird.y <= 0;

   		if(floorCondition || zuoShangJiaoCondition || youShangJiaoCondition || zuoXiaJiaoCondition || youXiaJiaoCondition || cellCondition){
   			//条件成立的话，碰撞了
   			//alert('碰撞了');
   			//取消循环调用函数
   			clearInterval(timer);
